    ...
    if (distance < 0) {
        ...
        if (countdownEl) {
            ...
            countdownEl.innerHTML =
                '<div style="text-align: center; color: var(--accent); font-family: \'Playfair Display\', serif; font-size: 1.3rem; font-weight: 600; text-shadow: 0 0 10px rgba(212, 175, 55, 0.3); width: 100%; margin: 0 auto; box-sizing: border-box; line-height: 1.5;">Haneefa & Rizwana\'s Wedding Has Begun! Alhamdulillah! 💍🎉</div>';
        }
        return;
    }
...
function launchMobileConfetti() {
    ...
    if (!cachedHeartShape) {
        cachedHeartShape = confetti.shapeFromText({ text: '❤️', scalar: 2 });
    }

    const isMobile = window.innerWidth < 768;

    // Reduce particle count on mobile for better performance and clean heart animation
...
function addToCalendar(title, location, datetime) {
    try {
        ...
        const googleCalendarUrl = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(title)}&dates=${formattedDate}/${endDate}&details=${encodeURIComponent('Haneefa & Rizwana Wedding Celebration - Join us for this blessed occasion!')}&location=${encodeURIComponent(location)}`;
